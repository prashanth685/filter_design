import numpy as np
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QScrollArea, QTableWidget, QTableWidgetItem, QLabel, QPushButton, QHBoxLayout
...
class TimeViewFeature:
    def __init__(self, parent, channel=None, model_name="model1", console=None):
        ...
        self.num_plots = 7  # Channels 1-4, tacho freq, tacho trigger, vpp vs tacho
        ...
        self.vpp_tacho_data = []  # Store (tacho_freq, vpp) pairs
    ...
    def initUI(self):
        ...
        for i in range(self.num_plots):
            # Only use TimeAxisItem for plots with time-based x-axis
            axis_items = {'bottom': TimeAxisItem(orientation='bottom')} if i < self.num_channels + 2 else None
            # Create PlotWidget with or without axisItems
            if axis_items:
            ...
            if i < self.num_channels:
                ...
                plot_widget.setLabel('left', 'Channel 2 Vpp (V)')
                plot_widget.setLabel('bottom', 'Tacho Frequency (Hz)')
            plot_widget.showGrid(x=True, y=True)
            plot_widget.addLegend()
            pen = mkPen(color=colors[i % len(colors)], width=2)
            plot = plot_widget.plot([], [], pen=pen, name=f'Plot {i+1}' if i < self.num_channels + 2 else 'Vpp vs Tacho')
        ...
        self.clear_button = QPushButton("Clear Vpp vs Tacho")
        ...
        self.clear_button.clicked.connect(self.clear_vpp_tacho_plot)
    ...
    def clear_vpp_tacho_plot(self):
        """Clear the Vpp vs Tacho Frequency plot."""
        self.vpp_tacho_data = []
        self.plots[self.num_channels + 2].setData([], [])
        logging.debug("Cleared Vpp vs Tacho Frequency plot")
        if self.console:
            self.console.append_to_console("Cleared Vpp vs Tacho Frequency plot")
    ...
    def calculate_vpp(self, data):
        ...
        vmax = np.max(data)
        vmin = np.min(data)
        logging.debug(f"vmax: {vmax}, vmin: {vmin}, vpp: {vmax-vmin}")
        return vmax - vmin
    ...
    def calculate_amplitude(self, data):
        ...
        logging.debug(f"Mean: {np.mean(data):.4f}, Centered Min: {np.min(data_centered):.4f}, Centered Max: {np.max(data_centered):.4f}, Amplitude: {amplitude:.4f}")
        return amplitude
    ...
    def on_data_received(self, tag_name, model_name, values, sample_rate):
        ...
        try:
            ...
            if len(self.data[1]) > 0:
                ...
                self.vpp_label.setText(f"Channel 2 Amplitude: {self.amplitude_ch2:.4f} V")
                ...
                # Update Vpp vs Tacho Frequency plot
                if len(self.data[self.num_channels]) > 0:
                    avg_tacho_freq = np.mean(self.data[self.num_channels])
                    self.vpp_tacho_data.append((avg_tacho_freq, self.vpp_ch2))
                    tacho_freqs = [x[0] for x in self.vpp_tacho_data]
                    vpps = [x[1] for x in self.vpp_tacho_data]
                    self.plots[self.num_channels + 2].setData(tacho_freqs, vpps)
                    if tacho_freqs:
                        self.plot_widgets[self.num_channels + 2].setXRange(min(tacho_freqs), max(tacho_freqs), padding=0.05)
                        self.plot_widgets[self.num_channels + 2].setYRange(min(vpps), max(vpps), padding=0.05)
            ...
            for ch in range(self.num_plots - 1):  # Exclude Vpp vs Tacho plot
