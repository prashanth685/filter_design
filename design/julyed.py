        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"values printed by {values}")
...
class TimeViewFeature:
    ...
    def initUI(self):
        ...
        colors = ['r', 'g', 'b']
        channel_labels = ['Channel 1 Input (V)', 'Channel 2 Output (V)']
        ...
        for i in range(self.num_plots):
            ...
            plot_widget.setMinimumWidth(150)
            if i < len(channel_labels):
                ...
                plot_widget.setYRange(10, -85, padding=0.1)
                plot_widget.setXRange(0, 1000, padding=0.1)
    ...
    def on_data_received(self, tag_name, model_name, values, sample_rate, samples_per_channel):
        ...
        try:
            ...
            # self.data[1] = np.array(values[1][:self.samples_per_channel], dtype=np.int16) *self.scaling_factor
            # values[1] is a list of uint16 values from channel 1
            raw_unsigned = np.array(values[1][:self.samples_per_channel], dtype=np.uint16)

            # Reinterpret the data as int16 (without changing the bits)
            signed_data = raw_unsigned.view(dtype=np.int16)

            # Optionally convert to float and scale
            self.data[1] = signed_data.astype(np.float32) * self.scaling_factor


            # voltage = (self.data / 32768.0) * 3.3
            # self.data[1] = np.array(values[1][:self.samples_per_channel]) * voltage
            print(f"channel 2 data {self.data[0]}")
            # start_index=100
            # end_index=min(4196,len(self.data[0]))
            # logging.debug(f"Channel 1 data [100:{end_index}]: {self.data[0][start_index:end_index]}")
