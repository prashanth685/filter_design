...
class TimeViewFeature:
    def __init__(self, parent, channel=None, model_name="model1", console=None):
        ...
        self.num_plots = 4  # Channel 1, Channel 2, Gain vs Input Freq, Log-x Plot
        ...
        self.gain_range = [0, -85]  # Default Gain range
    ...
    def initUI(self):
        ...
        for i in range(self.num_plots):
            ...
            if i < self.num_channels:
                ...
                plot_widget.setYRange(0, -85, padding=0)
            ...
            if i == 2:
                ...
                self.gain_slider.setMinimum(-8500)
                self.gain_slider.setMaximum(0)
                ...
                self.input_freq_slider.setMinimum(100)
                ...
                self.input_freq_slider.setMinimumWidth(300)
    ...
    def connect_buttons(self):
        ...
        self.freq_100hz_button.clicked.connect(lambda: self.set_input_freq_range(100))
        self.freq_1000hz_button.clicked.connect(lambda: self.set_input_freq_range(1000))
        ...
        self.data_range_slider.valueChanged.connect(self.update_data_range)
    ...
    def clear_gain_vs_freq_plot(self):
        ...
        self.plot_widgets[2].setYRange(self.gain_range[0], self.gain_range[1], padding=0)
    ...
    def set_input_freq_range(self, freq):
        ...
        self.input_freq_slider.setMaximum(freq)
        self.input_freq_slider.setValue(freq)
    ...
    def update_data_range(self, value):
        self.data_range_end = value
        if self.data_range_end < self.data_range_start + 100:
            self.data_range_end = self.data_range_start + 100
        self.apply_data_range()
        logging.debug(f"Data range updated to {self.data_range_start}-{self.data_range_end}")
    ...
    def apply_data_range(self):
        ...
        if len(self.data[3][0]) > 0:
            freqs = np.fft.fftfreq(self.channel_samples, d=1/self.sample_rate)
            positive_freqs = freqs[:self.channel_samples//2][self.data_range_start:self.data_range_end]
            valid_freqs = positive_freqs[positive_freqs > 0]
            if len(valid_freqs) > 0:
                fft_ch1 = np.abs(np.fft.fft(self.data[0]))[:self.channel_samples//2][self.data_range_start:self.data_range_end][positive_freqs > 0]
                fft_ch2 = np.abs(np.fft.fft(self.data[1]))[:self.channel_samples//2][self.data_range_start:self.data_range_end][positive_freqs > 0]
    ...
    def apply_ranges(self):
        ...
        logging.debug(f"Applied ranges: Gain {self.gain_range}, Input Freq {self.input_freq_range}")
        if self.console:
            self.console.append_to_console(f"Applied ranges: Gain {self.gain_range}, Input Freq {self.input_freq_range}")
    ...
    def on_data_received(self, tag_name, model_name, values, sample_rate):
        ...
        try:
            ...
            if len(self.data[0]) > 0 and len(self.data[1]) > 0:
                self.vrms_ch1 = self.calculate_vrms(self.data[0])
                self.vrms_ch2 = self.calculate_vrms(self.data[1])
                self.vrms_label.setText(f"Channel 1 Vrms: {self.vrms_ch1:.2f} V | Channel 2 Vrms: {self.vrms_ch2:.2f} V")
                self.frequency_ch2 = self.calculate_frequency(self.data[1], self.sample_rate)
                self.frequency_label.setText(f"Channel 2 Frequency: {self.frequency_ch2:.2f} Hz")
                input_frequency = self.calculate_frequency(self.data[0], self.sample_rate)
                gain_db = self.calculate_gain(self.vrms_ch1, self.vrms_ch2)
