...
def spectral_derivative(signal, dt):
    n = len(signal)
    freqs = np.fft.fftfreq(n, d=dt)
    fft_signal = np.fft.fft(signal)
    derivative_fft = 1j * 2 * np.pi * freqs * fft_signal
    return np.fft.ifft(derivative_fft).real
...
class TFilterApp:
    ...
    def create_gui(self):
        ...
        filter_types = ["lowpass", "highpass", "bandpass", "bandstop", "rc_integrator", "differentiator"]
    ...
    def design_filter(self):
        try:
            ...
            if filter_type in ["lowpass", "highpass", "bandpass", "bandstop"]:
                if fpass <= 0 or fstop <= 0 or order <= 0:
                    messagebox.showerror("Error", "All numerical inputs must be positive for this filter type.")
                    return
                ...
                if filter_type in ["bandpass", "bandstop"] and fstop <= fpass:
                    ...
                    return
            elif filter_type in ["rc_integrator", "differentiator"] and order < 1:
                messagebox.showerror("Error", "Filter order must be at least 1.")
                return
            ...
            elif filter_type == "rc_integrator":
                # Approximate RC integrator with a ramp-like impulse response
                t = np.arange(order + 1) / fs
                rc_time_constant = 1 / (2 * np.pi * fpass)  # Approximate time constant based on passband
                self.taps = t / rc_time_constant  # Linear ramp scaled by time constant
                self.taps = self.taps / np.sum(np.abs(self.taps))  # Normalize
            elif filter_type == "differentiator":
                # Generate a unit impulse for differentiation
                impulse = np.zeros(order + 1)
                impulse[0] = 1.0
                # Compute spectral derivative
                dt = 1 / fs
                self.taps = spectral_derivative(impulse, dt)
    ...
    def plot_response(self, plot_type):
        ...
        if plot_type == "frequency":
            ...
            freq = w * fs / (2 * np.pi)
            self.ax1.plot(freq, 20 * np.log10(np.abs(h) + 1e-10))  # Add small offset to avoid log(0)
            self.ax1.set_title(f"Frequency Response - {self.filter_type.get().replace('_', ' ').capitalize()}")
            ...
            self.ax1.stem(np.arange(len(self.taps)), self.taps, use_line_collection=True)
            self.ax1.set_title(f"Impulse Response - {self.filter_type.get().replace('_', ' ').capitalize()}")
...
// Filter Type: {self.filter_type.get().replace('_', ' ')}
